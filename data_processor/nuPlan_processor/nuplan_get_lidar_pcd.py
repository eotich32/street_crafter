...
def export_lidar_pcd(nuplan_root, log_name, save_dir, skip_existing=False):
    ...
    for cam_idx, cam_ts in enumerate(tqdm(cam_f0_ts_list, desc='CAM_F0 matched LiDAR')):
        ...
        if frame_key in track_info:
            for track_id, item in track_info[frame_key].items():
                box = item['lidar_box']
        ...
        for cam in db.camera:
            ...
            img_path = os.path.join(save_dir, 'images', f'{cam_idx:06d}_{cam_id}.jpg')
            ...
            cv2.imwrite(os.path.join(depth_dir, f"{cam_idx:06d}_{cam_id}.jpg"), overlay)
