...
def build_trajectory():
    ...
    active_tracks = {}  # TID -> {'kf': KalmanFilter, 'frame_idx': int, 'miss_cnt': int}
    MAX_MISS = 5
    DT = 0.2  # 5 Hz

    #  Helper：创建 3D 匀速卡尔曼滤波器
    def create_kf(initial_box):
        kf = KalmanFilter(dim_x=11, dim_z=7)
        # 状态 [x,y,z,l,w,h,yaw,vx,vy,vz,vyaw]
        kf.x = np.zeros(11)
        kf.x[:7] = initial_box
        # 观测矩阵
        kf.H = np.zeros((7, 11))
        ...
        kf.F[6, 10] = DT
        # 观测噪声
        kf.R[0:3, 0:3] *= 0.1 ** 2  # 位置
        kf.R[3:6, 3:6] *= 0.05 ** 2  # 尺寸
        kf.R[6, 6] *= 0.1 ** 2  # 角度
        # 过程噪声
        q_pos = Q_discrete_white_noise(2, DT, 0.1)
        q_size = Q_discrete_white_noise(2, DT, 0.01)
        q_yaw = Q_discrete_white_noise(2, DT, 0.05)
        kf.Q[0:2, 0:2] = q_pos
        kf.Q[3:5, 3:5] = q_pos
        kf.Q[6, 6] = q_yaw[0, 0]
        kf.Q[7:9, 7:9] = q_pos
        kf.Q[9, 9] = q_pos[0, 0]
        kf.Q[10, 10] = q_yaw[0, 0]
        # 初始协方差
        kf.P[0:7, 0:7] *= 1.0
        kf.P[7:, 7:] *= 0.1
        return kf
    ...
    def match_truth_boxes(tracks, detections):
        ...
        cost_matrix = np.zeros((len(tracks), len(detections)))
        for i, tid in enumerate(tid_list):
            box1 = tracks[tid]['kf'].x[:7]
            for j, det in enumerate(detections):
                box2 = det['box']
                dist = np.linalg.norm(box1[:3] - box2[:3])
                size_diff = np.abs(box1[3:6] - box2[3:6]).sum()
                dyaw = abs(box1[6] - box2[6])
                dyaw = min(dyaw, 2 * np.pi - dyaw)
                cost_matrix[i, j] = dist + 0.3 * size_diff + 0.5 * dyaw
        row_ind, col_ind = linear_sum_assignment(cost_matrix)
        matched, unmatched_tracks, unmatched_dets = [], tid_list.copy(), list(range(len(detections)))
        for r, c in zip(row_ind, col_ind):
            if cost_matrix[r, c] < 2.5:
                matched.append((tid_list[r], c))
                unmatched_tracks.remove(tid_list[r])
                unmatched_dets.remove(c)
        return matched, unmatched_tracks, unmatched_dets

    for idx, frame in enumerate(tqdm(FRAMES, desc="KF tracking")):
        fid = f"{idx:06d}"
        track_info[fid] = {}
        track_cam_vis[fid] = {i: [] for i in range(len(CAMERA_NAMES))}
        ...
        # 1. 先预测所有活跃轨迹
        for tid in list(active_tracks.keys()):
            kf = active_tracks[tid]['kf']
            # 角度平滑：转到复数域
            yaw = kf.x[6]
            c, s = math.cos(yaw), math.sin(yaw)
            kf.x[6] = c
            kf.x = kf.F @ kf.x
            new_c, new_s = kf.x[6], kf.x[10]
            kf.x[6] = math.atan2(new_s, new_c)
            active_tracks[tid]['frame_idx'] = idx

        if is_truth_frame:
            names, boxes = frame['annos']['names'], frame['annos']['boxes_3d']
            detections = [{'label': n.lower(), 'box': b} for n, b in zip(names, boxes)]
            matched, unmatched_tracks, unmatched_dets = match_truth_boxes(
            ...
            for tid, det_idx in matched:
                ...
                yaw_pred = kf.x[6]
                dy = yaw_obs - yaw_pred
                dy = dy - 2 * np.pi if dy > np.pi else (dy + 2 * np.pi if dy < -np.pi else dy)
                z = np.array(box)
                z[6] = yaw_pred + dy
                kf.update(z)
                active_tracks[tid]['miss_cnt'] = 0
