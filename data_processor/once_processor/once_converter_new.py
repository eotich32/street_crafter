...
def build_trajectory():
    ...
    # ---------- 剔除单帧 track ----------
    trajectory = {tid: tr for tid, tr in trajectory.items() if len(tr['frames']) > 1}

    # 同步过滤 track_camera_visible.pkl 里的记录
    for fid, cam_dict in track_cam_vis.items():
        for cam_i, vis_list in cam_dict.items():
            vis_list[:] = [tid for tid in vis_list if tid in trajectory]  # 原地剔除

    #再剔除 track_info 里这些 tid
    for fid, frame_dict in track_info.items():
        bad_tids = [tid for tid in frame_dict if tid not in trajectory]
        for tid in bad_tids:
            del frame_dict[tid]
    # 去掉空帧
    track_info = {k: v for k, v in track_info.items() if v}
...
def build_trajectory():
    ...
    # ---------- 预加载所有 ego pose（world→vehicle） ----------
    ego_poses = {}
    for idx, frame in enumerate(FRAMES):
        q = np.array(frame['pose'][:4])
        t = np.array(frame['pose'][4:])
        T_w2v = np.eye(4)
        T_w2v[:3, :3] = R.from_quat(q).as_matrix()
        T_w2v[:3, 3] = t
        ego_poses[idx] = T_w2v  # 世界→车辆
    ...
    for idx, frame in enumerate(tqdm(FRAMES, desc="truth-only tracking")):
        ...
        # 写入 trajectory & track_info（world 坐标系）
        for tid, det_idx in matched:
            det = curr_detections[det_idx]
            box = det['box']
            if tid not in trajectory:
            ...
            # vehicle→world
            box_v = np.array(box[:3])
            yaw_v = box[6]
            T_v2o = np.eye(4)
            T_v2o[:3, :3] = R.from_euler('z', yaw_v).as_matrix()
            T_v2o[:3, 3] = box_v
            T_w2o = ego_poses[idx] @ T_v2o
            t_w = T_w2o[:3, 3]
            #yaw_w = R.from_matrix(T_w2o[:3, :3]).as_euler('z')[0]
            #yaw_w = R.from_matrix(T_w2o[:3, :3]).as_euler('Z')[0]
            yaw_w = R.from_matrix(T_w2o[:3, :3]).as_rotvec()[2]

            pose = np.eye(4)
            pose[:3, :3] = R.from_euler('z', yaw_w).as_matrix()
            pose[:3, 3] = t_w
            trajectory[tid]['poses_vehicle'].append(pose.astype(np.float32))
            trajectory[tid]['frames'].append(idx)
            trajectory[tid]['timestamps'].append(float(frame['frame_id']))
            ...
            track_info[fid][tid] = {
                'lidar_box': {
                    'center_x': float(t_w[0]),
                    'center_y': float(t_w[1]),
                    'center_z': float(t_w[2]),
                    'length': box[3],
                    'width': box[4],
                    'height': box[5],
                    'heading': float(yaw_w),
    ...
    for fid, cam_dict in track_cam_vis.items():
        for cam_i, vis_list in cam_dict.items():
            vis_list[:] = [tid for tid in vis_list if tid in trajectory]
    for fid, frame_dict in track_info.items():
        bad_tids = [tid for tid in frame_dict if tid not in trajectory]
        for tid in bad_tids:
            del frame_dict[tid]
...
# ---------- 6. GPU 动态掩码（world 坐标版） ----------
# ---------- 6. GPU 动态掩码（world 坐标版，修复越界） ----------
def save_dynamic_mask_gpu(trajectory, track_cam_vis):
    ...
    for idx, frame in enumerate(tqdm(FRAMES, desc="dynamic_mask")):
        fid = f"{idx:06d}"
        T_world2ego = np.loadtxt(f"{OUTPUT_DIR}/ego_pose/{fid}.txt")
        T_ego2world = np.linalg.inv(T_world2ego)

        for cam_i in range(len(CAMERA_NAMES)):
            mask = np.zeros((IMAGE_SIZE[1], IMAGE_SIZE[0]), dtype=np.uint8)
            K, _ = read_intrinsic(cam_i)
            T_velo2cam = np.linalg.inv(
                np.loadtxt(f"{OUTPUT_DIR}/extrinsics/{cam_i}.txt")
            )
            T_world2cam = T_velo2cam @ T_ego2world
            ...
            for tid in track_cam_vis[fid][cam_i]:
                ...
                pose_world = traj['poses_vehicle'][pose_idx]
                ...
                corners_world = (pose_world[:3, :3] @ corners_obj.T).T \
                                + pose_world[:3, 3]

                # 全部投影到图像
                pts_cam = (T_world2cam[:3, :3] @ corners_world.T).T \
                          + T_world2cam[:3, 3]
                pts_2d = (K @ pts_cam.T).T
                uv_full = pts_2d[:, :2] / pts_2d[:, 2:3]

                # 标记是否在图像内
                inside = (uv_full[:, 0] >= 0) & (uv_full[:, 0] < IMAGE_SIZE[0]) & \
                         (uv_full[:, 1] >= 0) & (uv_full[:, 1] < IMAGE_SIZE[1])

                # 画面片
                for f in faces:
                    if all(inside[i] for i in f):
                        cv2.fillPoly(mask,
                                     [uv_full[f, :].astype(np.int32)],
                                     color=255)
...
def save_track_vis(trajectory, track_cam_vis):
    ...
    for idx, frame in enumerate(tqdm(FRAMES, desc="track_vis_local")):
        ...
        for tid in track_cam_vis[fid][cam_idx]:
            ...
            # ① 从 track_info 拿 ego 系真值框
            with open(f"{OUTPUT_DIR}/track/track_info.pkl", 'rb') as f:
                info = pickle.load(f)
            box = info[fid][tid]['lidar_box']
            ...
            # ② 构造 ego→obj 矩阵
            T_v2o = np.eye(4)
            T_v2o[:3, :3] = R.from_euler('z', yaw).as_matrix()
            T_v2o[:3, 3] = [cx, cy, cz]
            ...
            corners_v = (T_v2o[:3, :3] @ corners_obj.T).T + T_v2o[:3, 3]

            # ④ ego→camera→图像
            pts_cam = (T_velo2cam[:3, :3] @ corners_v.T).T + T_velo2cam[:3, 3]
            pts_2d = (K @ pts_cam.T).T
            ...
            cv2.putText(img, tid, tuple(center),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

        vis_frames.append(img[:, :, ::-1])

    imageio.mimwrite(f"{OUTPUT_DIR}/track/track_vis_local.mp4", vis_frames, fps=5)
# ---------- main ----------
def main():
    global FRAMES
    ...
    FRAMES = FRAMES[::2]          # 0, 2, 4, 6, ...
    if not FRAMES:
        print("⚠️  跳帧后无数据，请检查 --start_idx / --end_idx")
        return
