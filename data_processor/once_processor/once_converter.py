...
def build_trajectory():
    ...
    T_ego2world_list,_ = load_ego_poses(OUTPUT_DIR)
    # -------------- 主循环 --------------
    for idx, frame in enumerate(tqdm(FRAMES, desc="truth-only tracking")):
        ...
        #print("frame:",fid)
        #T_ego2world = ego_pose_matrix(frame)
        #T_ego2world_list,_ = load_ego_poses(OUTPUT_DIR)
        #if fid=="000000" or fid=="000001":
            #print("T_ego2world:",fid," ",T_ego2world)

        T_ego2world = T_ego2world_list[int(fid)]
        ...
        curr_detections = [{'label': name.lower(), 'box': box}
        ...
        # 4. 写全局 trajectory & track_info（同一帧索引，坐标系各自独立）
        for tid, det_idx in matched:
            det = curr_detections[det_idx]
            box = det['box']

            # --- 坐标计算 ---
            #center_lidar = np.array(box[:3])                      # 车体系
            #center_world = (T_ego2world @ np.r_[center_lidar, 1.0])[:3]
            #center_world = (np.r_[center_lidar, 1.0]@T_ego2world.T)[:3]
            center_world = np.array(box[:3])  

            # --- track_info：世界坐标 ---
            if tid not in track_info[fid]:
                track_info[fid][tid] = {}
            track_info[fid][tid]['lidar_box'] = {
                'center_x': float(-1*center_world[1]),
                'center_y': float(center_world[0]),
            ...
            trajectory[tid]['frames'].append(idx)        # ← 同一帧号
    ...
    # -------------- 后处理：只保留 ≥2 帧的轨迹 --------------
    trajectory = {tid: tr for tid, tr in trajectory.items() if len(tr['frames']) > 10}
    ...
    track_info = {k: {tid: info for tid, info in frame_dict.items() if tid in trajectory}
                  for k, frame_dict in track_info.items() if frame_dict}
