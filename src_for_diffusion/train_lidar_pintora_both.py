...
from my_utils.training_utils import parse_args_paired_training, UniRefDataset
...
def main(args):
    ...
    if args.pretrained_model_name_or_path == "stabilityai/sd-turbo":
        net_pix2pix = LidarPintoraConvIn(pretrained_path='/src/51sim-ai/img2img-turbo/output/pix2pix_turbo/lidar_pintora_corrupted_both/checkpoints/model_37501_waymoext.pkl', lora_rank_unet=args.lora_rank_unet, lora_rank_vae=args.lora_rank_vae)
    ...
    layers_to_opt += net_pix2pix.fusion_module.parameters()  # 加上融合注意力的参数
    ...
    dataset_train = UniRefDataset(dataset_folder=args.dataset_folder, image_prep=args.train_image_prep, split="train", tokenizer=net_pix2pix.tokenizer)
    dl_train = torch.utils.data.DataLoader(dataset_train, batch_size=args.train_batch_size, shuffle=True, num_workers=args.dataloader_num_workers)
    dataset_val = UniRefDataset(dataset_folder=args.dataset_folder, image_prep=args.test_image_prep, split="test", tokenizer=net_pix2pix.tokenizer)
    ...
    progress_bar = tqdm(range(29005, args.max_train_steps), initial=0, desc="Steps",
    ...
    for epoch in range(0, args.num_training_epochs):
        for step, batch in enumerate(dl_train):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                # forward pass
                x_tgt_pred = net_pix2pix(x_src, x_combined, prompt_tokens=batch["input_ids"], deterministic=True)
                # Reconstruction loss
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.eval_freq == 1:
                        ...
                        for step, batch_val in enumerate(dl_val):
                            ...
                            with torch.no_grad():
                                # forward pass
                                x_tgt_pred = accelerator.unwrap_model(net_pix2pix)(x_src, x_combined, prompt_tokens=batch_val["input_ids"].cuda(), deterministic=True)
                            ...
                            if args.track_val_fid:
                                img_src = transforms.ToPILImage()(x_src[0].cpu())
                                img_ref = transforms.ToPILImage()(x_combined[0].cpu())
                                img_tgt_pred = transforms.ToPILImage()(x_tgt_pred[0].cpu() * 0.5 + 0.5)
                                combined_width = img_src.width * 3
                                combined_height = img_src.height
                                combined_img = Image.new('RGB', (combined_width, combined_height))
                                # 拼接图像
                                combined_img.paste(img_src, (0, 0))
                                combined_img.paste(img_tgt_pred, (img_src.width, 0))
                                combined_img.paste(img_ref, (img_src.width * 2, 0))
