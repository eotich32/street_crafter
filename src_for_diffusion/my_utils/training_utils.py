...
def build_transform(image_prep):
    ...
    if image_prep == "resized_crop_512":
        ...
        T = transforms.Compose([
            transforms.Resize((576, 1024), interpolation=Image.LANCZOS),
    ...
    elif image_prep in ["resize_1024", "resize_576"]:
        T = transforms.Compose([
            transforms.Resize((576, 1024), interpolation=Image.LANCZOS)
        ])
...
class UniRefDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_folder, split, image_prep, tokenizer):
        super().__init__()
        self.ref_folder = os.path.join(dataset_folder, "lidars") #所有场景文件所在文件夹
        if split == "train":
        ...
        self.img_names = sorted(list(self.captions.keys()))
    ...
    def __getitem__(self, idx):
        ...
        ref_img = Image.open(os.path.join(self.ref_folder, img_name)).convert('RGB')
        
        width, height = input_img.size
        if height == 1066:
            input_img = input_img.crop((0, 166, 1600, 1066))
            ref_img = ref_img.crop((0, 166, 1600, 1066))
        width, height = output_img.size
        if height == 1066:
            output_img = output_img.crop((0, 166, 1600, 1066))

        #input_img = input_img.resize((1024, 576), Image.LANCZOS)
        #ref_img = ref_img.resize((1024, 576), Image.LANCZOS)

        # input images scaled to 0,1
        img_t = self.T(input_img)
        img_t = F.to_tensor(img_t)
        ref_img = self.T(ref_img)
        ref_img = F.to_tensor(ref_img)
        # output images scaled to -1,1
        output_t = self.T(output_img)
        output_t = F.to_tensor(output_t)

        if 'train' in self.input_folder:
            # 随机水平翻转
            flip = torch.rand(1) > 0.5
            # 应用同步翻转
            if flip:
                img_t = torch.flip(img_t, dims=[-1])  # 左右翻转最后一维（宽度）
                output_t = torch.flip(output_t, dims=[-1])
                ref_img = torch.flip(ref_img, dims=[-1])
...
class WaymoRefDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_folder, split, image_prep, tokenizer):
        super().__init__()
        self.ref_folder = '/src/51sim-ai/street_crafter/waymo_data' #所有场景文件所在文件夹
        if split == "train":
            self.input_folder = os.path.join(dataset_folder, "train_A")
            self.output_folder = os.path.join(dataset_folder, "train_B")
            captions = os.path.join(dataset_folder, "train_prompts.json")
    ...
    def __getitem__(self, idx):
        ...
        scene_idx = 49 #场景号
        ref_name = img_name.replace('.png', '_0.png')
        ref_path = os.path.join(self.ref_folder, str(scene_idx).zfill(3), 'lidar', 'color_render', ref_name)
        ref_img = Image.open(ref_path).convert('RGB')
        #mask_name = img_name + '_0_mask.png'
        #mask_path = os.path.join(self.ref_folder, str(scene_idx).zfill(3), 'lidar', 'color_render', mask_name)
...
class LidarRefDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_folder, split, image_prep, tokenizer):
        super().__init__()
        self.ref_folder = '/mnt/data/3d_reconstruction/data/nuscenes_100' #所有场景文件所在文件夹
        if split == "train":
            self.input_folder = os.path.join(dataset_folder, "train_A")
            self.output_folder = os.path.join(dataset_folder, "train_B")
            captions = os.path.join(dataset_folder, "train_prompts.json")
    ...
    def __getitem__(self, idx):
        ...
        ref_name = img_name.split('_')[1] + '_' + img_name.split('_')[2]
        if '.png' not in ref_name:
            ref_name = ref_name + '.png'
        ref_path = os.path.join(self.ref_folder, str(scene_idx).zfill(3), 'lidar', 'color_render', ref_name)
...
class RefDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_folder, split, image_prep, tokenizer):
        super().__init__()
        self.ref_folder = os.path.join(dataset_folder, "train_B")
        if split == "train":
            self.input_folder = os.path.join(dataset_folder, "train_A")
            self.output_folder = os.path.join(dataset_folder, "train_B")
            captions = os.path.join(dataset_folder, "train_prompts.json")
        ...
        with open(captions, "r") as f:
            self.captions = json.load(f)
        self.img_names = sorted(list(self.captions.keys()))
        ref_json = os.path.join(dataset_folder, "ref_prompts.json")
    ...
    def __getitem__(self, idx):
        ...
        ref_num = len(self.ref_dict[img_name])
        ref_name = self.ref_dict[img_name][randint(0, ref_num - 1)]
        ref_img = Image.open(os.path.join(self.ref_folder, ref_name))

        # input images scaled to 0,1
        img_t = self.T(input_img)
        img_t = F.to_tensor(img_t)
        ref_img_in = self.T(ref_img)
        ref_img_in = F.to_tensor(ref_img_in)
        # output images scaled to -1,1
        output_t = self.T(output_img)
        output_t = F.to_tensor(output_t)
        ref_img_out = self.T(ref_img)
        ref_img_out = F.to_tensor(ref_img_out)
        ...
        if 'train' in self.input_folder:
            ...
            if flip:
                img_t = torch.flip(img_t, dims=[-1])  # 左右翻转最后一维（宽度）
                output_t = torch.flip(output_t, dims=[-1])
                ref_img_in = torch.flip(ref_img_in, dims=[-1])
                ref_img_out = torch.flip(ref_img_out, dims=[-1])

        output_t = F.normalize(output_t, mean=[0.5], std=[0.5])
        ref_img_out = F.normalize(ref_img_out, mean=[0.5], std=[0.5])
        ...
        return {
            "output_pixel_values": output_t,
            "conditioning_pixel_values": img_t,
            "ref_input": ref_img_in,
            "ref_output": ref_img_out,
            "caption": caption,
            "input_ids": input_ids,
        }
