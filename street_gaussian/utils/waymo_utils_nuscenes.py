...
import yaml
# 转换nuScenes坐标处理
def load_ego_ori_center(datadir):
...
def transform_box_center(box_center_global, ego_pose):
    ego_translation = ego_pose[:3, 3]
    transformed_center = box_center_global - ego_translation
    return transformed_center
config_path = '/src/51sim-ai/street_crafter/configs/pairs_new.yaml' #在nuscenes上需在source_path注明处理后场景位置
with open(config_path, 'r') as f:
    config = yaml.safe_load(f)
#ori_ego_frame_poses, ori_ego_cam_poses = load_ego_ori_center('/src/51sim-ai/street_crafter/processed_data/061') 
ori_ego_frame_poses, ori_ego_cam_poses = load_ego_ori_center(config['source_path'])
# calculate obj pose in world frame
# box_info: box_center_x box_center_y box_center_z box_heading
def make_obj_pose(ori_ego_pose, ego_pose, box_info):
    ...
    transformed_center = transform_box_center(np.array([tx, ty, tz]),ori_ego_pose)
    #obj_pose_vehicle[:3, 3] = np.array([tx, ty, tz])
    obj_pose_vehicle[:3, 3] = np.array([transformed_center[0], transformed_center[1], transformed_center[2]])
    obj_pose_vehicle[:3, 3] = obj_pose_vehicle[:3, 3] + ego_pose[:3, 3]
    obj_pose_world = obj_pose_vehicle
    ...
    if len(object_info) == 0:
        ...
        for i in range(len(cams)):
            ...
            for k, v in object_info.items():
                ...
                ori_ego_pose = ori_ego_cam_poses[cam, frame] #未减去中心点的世界坐标系ego
                pose_vehicle, pose_world = make_obj_pose(ori_ego_pose, ego_pose, pose)
