...
def quaternion_raw_multiply_theta(a: torch.Tensor, theta: torch.Tensor) -> torch.Tensor:
    ...
    bw, bz = torch.cos(theta / 2), torch.sin(theta / 2)
...
def quaternion_raw_multiply(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    # a = roma.quat_wxyz_to_xyzw(a)
    # b = roma.quat_wxyz_to_xyzw(b)
    # o = roma.quat_product(a, b)
    # o = roma.quat_xyzw_to_wxyz(o)
    # return o
    # # A = torch.stack([
    # #     torch.stack([aw, -ax, -ay, -az], dim=-1),
    # #     torch.stack([aw, ax, ay, -az], dim=-1),
    # #     torch.stack([aw, -ax, ay, az], dim=-1),
    # #     torch.stack([aw, ax, -ay, az], dim=-1),],
    # #     dim=-2)
    # M = torch.stack([
    #     torch.stack([bw, -bx, -by, -bz], dim=-1),
    #     torch.stack([bx, bw, bz, -by], dim=-1),
    #     torch.stack([by, -bz, bw, bx], dim=-1),
    #     torch.stack([bz, by, -bx, bw], dim=-1),],
    #     dim=-2)
    # x = a.unsqueeze(-1)
    # O = torch.matmul(M, x).squeeze(-1)
    # # print((S - O).abs().max(), 'ddd')
    # return O
...
def quaternion_slerp(q0: torch.Tensor, q1: torch.Tensor, step=0.5) -> torch.Tensor:
    ...
    q = roma.quat_xyzw_to_wxyz(q).squeeze(0)
