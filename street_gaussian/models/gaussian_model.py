...
from street_gaussian.utils.general_utils import inverse_sigmoid, get_expon_lr_func, quaternion_to_matrix
from street_gaussian.utils.sh_utils import RGB2SH
from street_gaussian.utils.graphics_utils import BasicPointCloud
from street_gaussian.utils.general_utils import strip_symmetric, build_scaling_rotation
...
class GaussianModel(nn.Module):
    def __init__(self, model_name='background', num_classes=1):
        ...
        self.max_screen_size = cfg.optim.max_screen_size
    ...
    def reset_optimizer(self, tensors_dict):
        ...
        for group in self.optimizer.param_groups:
            if group['name'] in name_list:
                ...
                if reset_tensor is not None and stored_state is not None:
    ...
    def densification_postfix(self, tensors_dict):
        ...
        self.xyz_gradient_accum = torch.cat([self.xyz_gradient_accum, torch.zeros(cat_points_num, 2, device='cuda', dtype=torch.float)], dim=0)
        self.denom = torch.cat([self.denom, torch.zeros(cat_points_num, 1, device='cuda', dtype=torch.float)], dim=0)
        self.max_radii2D = torch.cat([self.max_radii2D, torch.zeros(cat_points_num, device='cuda', dtype=torch.float)], dim=0)
